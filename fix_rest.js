...
async function main(){
  // Use raw RPC to bypass schema cache
  const {data,error} = await supabase.rpc('exec_sql', {sql: "UPDATE profiles SET role='supervisor' WHERE email IN ('hoyekjp@gmail.com','miladaali995@gmail.com') RETURNING email,role"});
  console.log("RPC result:", data, error?.message);

  // Fallback: try direct update with no-cache header
  const r1 = await fetch("https://gxmdtemgiuvahlcaobrr.supabase.co/rest/v1/profiles?email=eq.hoyekjp@gmail.com", {
    method: "PATCH",
    headers: {
  ...
  console.log("JP REST update:", JSON.stringify(j1));

  const r2 = await fetch("https://gxmdtemgiuvahlcaobrr.supabase.co/rest/v1/profiles?email=eq.miladaali995@gmail.com", {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      "apikey": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imd4bWR0ZW1naXV2YWhsY2FvYnJyIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3NjkzMjYyMCwiZXhwIjoyMDkyNTA4NjIwfQ.S3Q-YJf_dlJqSPKebfNWFpwUo-2_x-Ci5llCK6_ueps",
  ...
  console.log("Miled REST update:", JSON.stringify(j2));
}
main();
