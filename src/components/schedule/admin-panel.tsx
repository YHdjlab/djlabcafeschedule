            ...
            {generatedSlots.map(slot => (
              <div key={slot.key} className={cn('p-3 rounded-xl border text-sm', slot.issues?.length ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200')}>
                ...
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {[
                    slot.supervisor_id && { id: slot.supervisor_id, role: 'Sup' },
                    slot.bar_staff_id && { id: slot.bar_staff_id, role: 'Bar' },
                    slot.floor_staff1_id && { id: slot.floor_staff1_id, role: 'Floor' },
                ...
                {slot.issues?.length > 0 && (
                  <p className="text-xs text-red-500 mt-1">? {slot.issues.join(', ')}</p>
                )}
              </div>
            ))}
          </div>
