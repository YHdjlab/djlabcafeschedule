              ...
              <div key={slot.key} className={cn('bg-white rounded-3xl shadow-sm w-full overflow-hidden', slot.issues?.length ? 'ring-2 ring-red-300' : 'ring-1 ring-black/[0.06]')}>
                ...
                <div className={cn('px-4 py-4 flex items-center justify-between gap-2', slot.issues?.length ? 'bg-red-900' : 'bg-[#323232]')} style={{minWidth:0}}>
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-base font-black text-white">{fmtH(slot.startH)} â€“ {fmtH(slot.endH)}</p>
                    <p className="text-xs text-white/40 mt-0.5">{slot.staff?.filter((m:any)=>m.role!=='Available').length || 0} assigned Â· {slot.issues?.length ? slot.issues.length + ' issue' + (slot.issues.length>1?'s':'') : 'all good'}</p>
                  </div>
                </div>
                      ...
                      <div key={member.id} className={cn("rounded-2xl border px-3 py-3 transition-all", roleBg, member.role === 'Available' && 'opacity-60')} style={{minWidth:0}}>
                        ...
                        <div className="flex items-center gap-2 mb-3 flex-nowrap">
                          ...
                          {member.role !== 'Available' && (
                            ...
                            }} className="w-6 h-6 rounded-full bg-black/5 hover:bg-red-50 hover:text-red-400 flex items-center justify-center flex-shrink-0 transition-all text-gray-300 text-xs font-bold">âœ•</button>
                          )}
                ...
                {slot.issues?.length > 0 && (
                  <div className="mx-6 mb-6 px-5 py-4 bg-red-50 border border-red-100 rounded-2xl flex items-start gap-3">
                    ...
                    <span className="text-xs text-red-500 font-medium">{slot.issues.join(' Â· ')}</span>
                  </div>
                )}
              </div>
            )
