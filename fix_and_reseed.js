...
async function main() {
  // Fix roles directly
  const { error: e1 } = await supabase.from("profiles").update({role:"supervisor"}).eq("email","hoyekjp@gmail.com");
  const { error: e2 } = await supabase.from("profiles").update({role:"supervisor"}).eq("email","miladaali995@gmail.com");
  console.log("Role fix JP:", e1 ? e1.message : "OK");
  console.log("Role fix Miled:", e2 ? e2.message : "OK");

  // Load real profile IDs
  ...
  for (const [email, blocks] of Object.entries(staffBlocks)) {
    ...
    else { console.log("Seeded", profile.full_name+":", Object.values(blocks).flat().length, "blocked hours"); total += rows.length; }
  }
  console.log("\nTotal slots:", total);
}
main();
