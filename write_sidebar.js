...
export function Sidebar({ profile }: { profile: Profile }) {
    ...
    <div className="flex flex-col h-full bg-white border-r border-black/5">
      <div className="p-5 border-b border-black/5">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-[#FF6357] flex items-center justify-center shadow-sm">
  ...
  return (
    <>
      ...
      <aside className="hidden lg:flex lg:flex-col fixed left-0 top-0 h-full w-60 z-30"><Inner/></aside>
      <aside className={cn('lg:hidden fixed left-0 top-0 h-full w-64 z-50 transition-transform duration-300', open ? 'translate-x-0' : '-translate-x-full')}><Inner/></aside>
    </>
  )
}
;
require('fs').writeFileSync('src/components/layout/sidebar.tsx', sidebarCode, 'utf8');
